...
const StarshipView = ({ match }) => {
  const starship = useFetchData(`https://swapi.dev/api/starships/${match.params.id}/`) ;
  const formatDataList = (content) =>  content.results.slice(0, 9);
  const starshipSlide = useFetchData("https://swapi.dev/api/starships/?format=json", formatDataList);
