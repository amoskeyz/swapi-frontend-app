...
const PlanetView = ({ match }) => {
  ...
  const planet = useFetchData(`https://swapi.dev/api/planets/${match.params.id}/`) ;
  const formatDataList = (content) =>  content.results.slice(0, 9);
  const planetSlide = useFetchData("https://swapi.dev/api/planets/?format=json", formatDataList);
