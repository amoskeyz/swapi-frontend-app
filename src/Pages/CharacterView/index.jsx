...
const CharacterView = ({ match }) => {
  const character = useFetchData(`https://swapi.dev/api/people/${match.params.id}/`) ;
  const formatDataList = (content) =>  content.results.slice(0, 9);
  const characterSlide = useFetchData("https://swapi.dev/api/people/?format=json", formatDataList);
